...
class AIReplyCog(commands.Cog):
    ...
    async def generate_ai_response(self, trigger_message, user_id, username, is_mod, additional_context="", voluntary=False, mod_command_detected=False):
        ...
        async for msg in trigger_message.channel.history(limit=15):
            ...
            context_messages.append(f"{author_info}: {content}")
        ...
        # NEW: Build channel context info with proper Discord links
        channel_info = ""
        if additional_context:
            # Extract channel mentions and format them properly
        ...
        full_prompt = (
            ...
            f"{channel_info}\n"
            f"User {username} (ID: {user_id}) just sent the last message.{mod_note}\n"
            "Reply helpfully and conversationally as Zippy."
        )
        if voluntary:
            full_prompt += "\nYou are speaking on your own because the conversation looked interesting. Keep it casual and natural like a normal Discord user."
...
- Always censor the word "acrylic" as "acr*lic" — no exceptions.
...
- Pay attention to who is being directly addressed. If someone pings you specifically to ask YOUR opinion (like "what do you think about X"), ignore other conversations and just focus on answering their question.
- If you're tagged into an ongoing conversation, then use the full context to participate naturally.
- Messages from "Zippy (Beta)" or "Zippy" are YOUR OWN past messages. Don't reference them as if they're from someone else.
- Look for conversation breaks: topic changes, time gaps, different participants = different conversations. Don't mix them up.
